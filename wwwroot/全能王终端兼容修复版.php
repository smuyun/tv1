    ...
    foreach ($目录项 as $项目) {
        ...
        if (is_dir($路径)) {
            ...
            if (in_array($扩展名, $文件类型)) {
                $相对路径 = str_replace('/storage/emulated/0/纯福利/', '', $路径);
                
                // 检查是否为磁力文件夹
                $是磁力文件夹 = (strpos($路径, '/纯福利/磁力/') !== false);
...
function 获取所有文件() {
    ...
    if ($所有文件 === null) {
        ...
        $JSON文件 = 递归扫描目录('/storage/emulated/0/纯福利/影视/', ['json']);
        $TXT文件 = 递归扫描目录('/storage/emulated/0/纯福利/福利/', ['txt']);
        $M3U文件 = array_merge(
            递归扫描目录('/storage/emulated/0/纯福利/福利/', ['m3u']),
            递归扫描目录('/storage/emulated/0/纯福利/影视/', ['m3u'])
        );
        
        // 增强数据库文件扫描，包含所有可能的数据库路径
        $数据库文件 = array_merge(
            递归扫描目录('/storage/emulated/0/纯福利/json/影视/', ['db', 'sqlite', 'sqlite3', 'db3']),
            递归扫描目录('/storage/emulated/0/纯福利/福利/', ['db', 'sqlite', 'sqlite3', 'db3']),
            递归扫描目录('/storage/emulated/0/纯福利/db/', ['db', 'sqlite', 'sqlite3', 'db3']),
            递归扫描目录('/storage/emulated/0/纯福利/影视/', ['db', 'sqlite', 'sqlite3', 'db3']),
            递归扫描目录('/storage/emulated/0/纯福利/data/', ['db', 'sqlite', 'sqlite3', 'db3']),
            递归扫描目录('/storage/emulated/0/纯福利/数据库/', ['db', 'sqlite', 'sqlite3', 'db3'])
        );
